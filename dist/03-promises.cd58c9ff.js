...
var _notiflixAio = require("notiflix/build/notiflix-aio");
const refs = {
...
function submitForm(e) {
    ...
    for(let i = 0; i < amount.value; i += 1){
        ...
        createPromise(i, delayCount).then((result)=>(0, _notiflixAio.Notify).success(result)).catch((error)=>(0, _notiflixAio.Notify).failure(error));
    }
}
function createPromise(position, delay) {
    return new Promise((result, reject)=>{
        ...
        setTimeout(()=>{
            if (shouldResolve) result(`✅ Fulfilled promise ${position} in ${delay}ms`);
            else reject(`❌ Rejected promise ${position} in ${delay}ms`);
        }, delay);
    });
}

},{"notiflix/build/notiflix-aio":"0Lbm5"}],"0Lbm5":[function(require,module,exports) {
var global = arguments[3];
/*
...
*/ /* global define */ (function(root, factory) {
    ...
    var blockCreateOrRemove = function(isCreate, blockType, selectorOrHTMLElements, messageOrOptions, options, delay) {
        ...
        if (!newBlockSettings) Notiflix.Block.init({});
        ...
        // check the message: end
        // if cssAnimaion is false => duration: begin
        if (!newBlockSettings.cssAnimation) newBlockSettings.cssAnimationDuration = 0;
        // if cssAnimaion is false => duration: end
        // check the class name: begin
        var blockClassName = blockSettings.className;
        if (typeof newBlockSettings.className === "string") blockClassName = newBlockSettings.className.trim();
        ...
        if (isCreate) {
            ...
            for(var queryIndex = 0; queryIndex < checkQueryLimit; queryIndex++){
                ...
                if (eachElement) {
                    if (voidAndUnavailableElements.indexOf(eachElement.tagName.toLocaleLowerCase("en")) > -1) break;
                    ...
                    if (eachBlockElement.length < 1) {
                        ...
                        var icon = "";
                        if (blockType) {
                            if (blockType === typesBlock.Hourglass) icon = loadingAndBlockSvgIconHourglass(newBlockSettings.svgSize, newBlockSettings.svgColor);
                            else if (blockType === typesBlock.Circle) icon = loadingAndBlockSvgIconCircle(newBlockSettings.svgSize, newBlockSettings.svgColor);
                            else if (blockType === typesBlock.Arrows) icon = loadingAndBlockSvgIconArrows(newBlockSettings.svgSize, newBlockSettings.svgColor);
                            else if (blockType === typesBlock.Dots) icon = loadingAndBlockSvgIconDots(newBlockSettings.svgSize, newBlockSettings.svgColor);
                            else if (blockType === typesBlock.Pulse) icon = loadingAndBlockSvgIconPulse(newBlockSettings.svgSize, newBlockSettings.svgColor);
                            else icon = loadingAndBlockSvgIconStandard(newBlockSettings.svgSize, newBlockSettings.svgColor);
                        }
                        ...
                        var messageElement = "";
                        if (message.length > 0) {
                            if (message.length > newBlockSettings.messageMaxLength) message = commonGetPlaintext(message).substring(0, newBlockSettings.messageMaxLength) + "...";
                            else message = commonGetPlaintext(message);
                        ...
                        var minHeightStyle = "";
                        if (averageMinHeight > eachElementHeight) minHeightStyle = "min-height:" + averageMinHeight + "px;";
                        // internal style: begin
                        var eachElementIdOrClass = "";
                        if (eachElement.getAttribute("id")) eachElementIdOrClass = "#" + eachElement.getAttribute("id");
                        else if (eachElement.classList[0]) eachElementIdOrClass = "." + eachElement.classList[0];
                        else eachElementIdOrClass = (eachElement.tagName || "").toLocaleLowerCase("en");
                        var positionStyle = "";
                        var positions = [
                            "absolute",
                            "relative",
                        ...
                        if (addPosition || minHeightStyle.length > 0) {
                            ...
                            if (addPosition) positionStyle = "position:relative!important;";
                            // create and add internal style to the head
                            var style = '<style id="Style-' + blockSettings.ID + "-" + blockCreateOrRemoveCounter + '">' + eachElementIdOrClass + "." + positionClassForNonStaticRef + "{" + positionStyle + minHeightStyle + "}" + "</style>";
        ...
        // add element(s) and style: end
            ...
            var removeBlockElements = function(eachOne) {
                var timeout = setTimeout(function() {
                    // remove element
                    if (eachOne.parentNode !== null) eachOne.parentNode.removeChild(eachOne);
                    ...
                    if (eachOneStyle && eachOneStyle.parentNode !== null) eachOneStyle.parentNode.removeChild(eachOneStyle);
                    // clear timeout
                    clearTimeout(timeout);
            ...
            var removeClassBlockElements = function(eachBlockElement) {
                // if elements exist
                if (eachBlockElement && eachBlockElement.length > 0) for(var i = 0; i < eachBlockElement.length; i++){
                ...
                else if (typeof selectorOrHTMLElements === "string") commonConsoleLog('"Notiflix.Block.remove();" function called with "' + selectorOrHTMLElements + '" selector, but this selector does not have a "Block" element to remove.');
                else commonConsoleLog('"Notiflix.Block.remove();" function called with "' + selectorOrHTMLElements + '", but this "Array<HTMLElement>" or "NodeListOf<HTMLElement>" does not have a "Block" element to remove.');
            };
    ...
    var Notiflix = {
        ...
        Block: {
            ...
            merge: function(userBlockExtendOptions) {
                // if initialized already
                if (newBlockSettings) newBlockSettings = commonExtendOptions(true, newBlockSettings, userBlockExtendOptions);
                else {
